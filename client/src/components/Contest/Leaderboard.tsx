...
const LeaderboardTab = ({ contestUsers }: LeaderboardTabProps) => {
	...
	// Add null checks for contestUsers
	if (!contestUsers || contestUsers.length === 0) {
		return <div className="bg-white p-4 rounded-2xl shadow-md w-full mx-auto mt-5">
			<p className="text-center text-gray-500">No contest users found</p>
		</div>;
	}

	// Check if the first user has problem status
	const firstUser = contestUsers[0];
	if (!firstUser || !firstUser.contestUserProblemStatus) {
		return <div className="bg-white p-4 rounded-2xl shadow-md w-full mx-auto mt-5">
			<p className="text-center text-gray-500">No problem status data available</p>
		</div>;
	}
	...
	const downloadCSV = () => {
		if (!firstUser || !firstUser.contestUserProblemStatus) {
			console.error("No problem status data available for CSV download");
			return;
		}
		...
		const headers = [
			...
			"Name",
			"Roll Number",
			...firstUser.contestUserProblemStatus.map(
				(_, idx) => `Problem ${idx + 1}`,
			),
			"Marks",
		];
				...
				<table className="min-w-full bg-white border border-gray-200 rounded-lg">
					<thead>
						<tr className="bg-secondary text-white font-semibold">
							...
							{firstUser.contestUserProblemStatus.map((problem) => (
