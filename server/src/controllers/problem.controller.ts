...
class ProblemController {
    async createProblem(req: Request, res: Response) {
        ...
        console.log("Problem ID type:", typeof problemId, "Value:", problemId);
        console.log("Editorial content:", problemEditorial);
        ...
        let editorialContent = (problemEditorial && problemEditorial.trim()) || "Editorial content will be added later.";
        console.log("Final editorial content:", editorialContent);
        
        // Ensure editorial content is not empty
        if (!editorialContent || editorialContent.trim() === "") {
            editorialContent = "Editorial content will be added later.";
        }
        ...
        console.log("Editorial object:", editorial);
        try {
            // Save both problem and editorial
            const [savedProblem, savedEditorial] = await Promise.all([
                problem.save(),
                editorial.save()
            ]);
            
            console.log("Problem and editorial saved successfully");
            ...
            // If there was an error, try to clean up any partially saved data
            try {
                await Problem.findOneAndDelete({ problemId: problemId });
                await Editorial.findOneAndDelete({ editorialId: problemId });
                console.log("Cleaned up any partially saved data");
            } catch (cleanupError) {
